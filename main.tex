...
\begin{compactdesc}
  \item[Latent vector model:] $L(\theta; \mathbf{w}) = \sum_{t=1}^T\sum_{\Delta \in I}[b_{w(t+\Delta)} + \langle \mathbf{x}_{w(t+\Delta)}, \mathbf{x}_{w(t)} \rangle - \log\sum_{v \in V}\exp[\langle \mathbf{x}_v, \mathbf{x}_{w(t)} \rangle + b_v]]$ 
\end{compactdesc}

\subsection{GloVe Objective}
\begin{compactdesc}
  \item $H(\theta;N) = \sum_{i,j:n_{ij}>0} f(n_{ij})(\log n_{ij} - \log \tilde{p}_\theta(w_j|w_i))$ and
  \item $\tilde{p}_\theta(w_j|w_i) = \exp[\langle x_i; y_j \rangle + b_i + d_j]$ \& $f(n) = \min\{1, (\frac{n}{n_{max}})^\alpha\}$
