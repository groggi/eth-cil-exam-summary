...
\begin{compactdesc}
	...
	\item[\# of free params:] fixed covariance matrix: $\kappa(\theta) = K \cdot D + (K - 1)$ ($K$ \# clusters, data dimension $D$; dimension of the $\mu$'s, $K-1$ \# free clusters), full covariance matrix: $\kappa(\theta) = K(D + \frac{D(D+1)}{2}) + (K - 1)$. Compare AIC/BIC for different $K$ -- the smaller the better. BIC penalizes complexity more.
\end{compactdesc}
...
\begin{compactdesc}
  ...
  \item[Latent Vector Model:] $w \mapsto (\mathbf{x}_w, b_w) \in \mathbb{R}^{D+1} \\p_{\theta}(w|w') = \frac{\exp[\langle \mathbf{x}_w,\mathbf{x}_{w'}\rangle + b_w]}{\sum_{v\in V}{\exp[\langle \mathbf{x}_v,\mathbf{x}_{w'}\rangle + b_v ]}}$ Log-L'd is hard to compute.
\end{compactdesc}
Simplify; distinguish vocabulary $V$ and context vocabulary $C$ $\rightarrow$ use mixed inner products. Change objective to GloVe.

\subsection{GloVe Objective}
\begin{compactdesc}
  \item[Co-occurence Matrix:]$\mathbf{N} = (n_{ij}) \in \mathbb{R}^{|V|\cdot |C|} \leftrightarrow \# w_i$ in c'txt $w_j$
  \item[Objective:] $H(\theta;\mathbf{N}) = \sum_{n_{ij} > 0} f(n_{ij})(\log n_{ij} - \log \exp[\langle \mathbf{x}_i, \mathbf{y}_j \rangle + b_i + d_j]$ with $f(n) = \min\{1, (\frac{n}{n_{max}})^\alpha\}$, $\alpha \in (0;1]$.
\end{compactdesc}
